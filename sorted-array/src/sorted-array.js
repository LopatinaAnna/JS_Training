...
const sortedByRating = () => {
    return Array.from(products).sort((a, b) => parseInt(b.ratingReviews) - parseInt(a.ratingReviews));
};

const sortedByPrice = () => {
    return Array.from(products).sort((a, b) => {
        getPrice = (x) => parseInt((typeof(x.price) === 'object' ? x.price.newUan : x.price).replace(/[\W]/g, ''));
